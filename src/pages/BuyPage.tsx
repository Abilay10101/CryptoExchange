...
export function BuyPage() {
  ...
  useEffect(() => {
    const fetchExchangeRates = async () => {
      try {
        // 1. Получаем курсы криптовалют через Binance
        const [btcResponse, ethResponse] = await Promise.all([
          fetch('https://api.binance.com/api/v3/ticker/price?symbol=BTCUSDT'),
          fetch('https://api.binance.com/api/v3/ticker/price?symbol=ETHUSDT')
        ]);
  
        // 2. Используем альтернативный API для KZT (с бесплатным ключом)
        const kztResponse = await fetch('https://api.twelvedata.com/exchange_rate?symbol=USD/KZT&apikey=your_api_key');
        print(kztData.rate);
        print("hola");
        ...
        // 3. Проверяем и логируем полученные данные
        console.log('BTC/USD:', btcData.price);
        console.log('ETH/USD:', ethData.price);
        console.log('USD/KZT:', kztData.rate);
  
        const btcPrice = parseFloat(btcData.price);
        const ethPrice = parseFloat(ethData.price);
        const kztRate = parseFloat(kztData.rate);
  
        // 4. Рассчитываем кросс-курсы
        const allRates = {
          // BTC
          'BTC-USD': btcPrice,
          'BTC-KZT': btcPrice * kztRate,
          'BTC-ETH': btcPrice / ethPrice,
          
          // ETH
          'ETH-USD': ethPrice,
          'ETH-KZT': ethPrice * kztRate,
          'ETH-BTC': ethPrice / btcPrice,
          
          // KZT
          'KZT-USD': 1 / kztRate,
          'KZT-BTC': 1 / (btcPrice * kztRate),
          'KZT-ETH': 1 / (ethPrice * kztRate),
        };
  
        console.log('Calculated ETH-KZT:', allRates['ETH-KZT']);
        setRates(allRates);
        setIsLoading(false);
      } catch (error) {
  ...
  const handleCurrencySelect = (currency: string, type: 'from' | 'to') => {
    ...
    const rate = getExchangeRate(rates, newFromCurrency, newToCurrency);
